...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mu = x.sum(dim=0).reshape(1, -1) / x.shape[0]
    sigma = (((x - mu) ** 2).sum(dim=0).reshape(1, -1) / (x.shape[0] - 1)) ** 0.5
    y = (x - mu) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    group_ids = torch.repeat_interleave(
        torch.arange(len(xs), device=ls.device), ls
    )

    y = torch.zeros(len(xs), dtype=flat.dtype, device=flat.device)
    y.scatter_add_(0, group_ids, flat)
    y = y / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    first = torch.ones(x.shape[0], dtype=torch.bool, device=x.device)
    first[1:] = sorted_x[1:] != sorted_x[:-1]

    uniques = sorted_x[first]
    indices = sorted_indices[first]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
