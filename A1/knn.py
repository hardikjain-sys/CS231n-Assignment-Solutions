...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_sq = (x_train_flat ** 2).sum(dim=1, keepdim=True)
    test_sq = (x_test_flat ** 2).sum(dim=1)

    dists = train_sq + test_sq - 2 * x_train_flat @ x_test_flat.t()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        values, indexes = torch.topk(dists[:, i], k, largest=False)
        nearest_labels = y_train[indexes]

        labels, counts = torch.unique(nearest_labels, return_counts=True)

        max_count = counts.max()
        y_pred[i] = labels[counts == max_count].min()      
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        for i in range(x_test.shape[0]):
          ...
          max_count = counts.max()
          y_test_pred[i] = labels[counts == max_count].min()      
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for fold in range(num_folds):
          ...
          train_folds = [x_train_folds[i] for i in range(num_folds) if i != fold]
          train_label_folds = [y_train_folds[i] for i in range(num_folds) if i != fold]

          x_train_fold = torch.cat(train_folds, dim=0)
          y_train_fold = torch.cat(train_label_folds, dim=0)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    best_k = min(
    k_to_accuracies,
    key=lambda k: (-sum(k_to_accuracies[k]) / len(k_to_accuracies[k]), k)
      )     
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
